...
% Last Modified by GUIDE v2.5 20-Mar-2020 10:35:32
...
axes(handles.axes1);
[x,map] = imread('Fondo Welcome.jpg');
image(x)
colormap(map);
axis off
hold on
...
% --- Executes on button press in CtrlAuto.
function CtrlAuto_Callback(hObject, eventdata, handles)
% hObject    handle to CtrlAuto (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
close(Welcome);
RobotC;
...
function CtrlManual_Callback(hObject, ~, handles)
